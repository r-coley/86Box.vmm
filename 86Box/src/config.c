...
#ifdef __APPLE__
static int
config_is_legacy_vmnet_host_device_name(const char *host_dev_name)
{
    ...
    if (p != NULL) {
        ...
        if (p != NULL) {
            if (!strcmp(p, "pcap") || !strcmp(p, "1"))
                ...
                nc->net_type = NET_TYPE_VMNET_HOST;
            else if (!strcmp(p, "vmnet-bridged") || !strcmp(p, "vmnet-bridge") || !strcmp(p, "9"))
    ...
    for (uint8_t c = 0; c < NET_CARD_MAX; c++) {
        ...
        switch(nc->net_type) {
            ...
            case NET_TYPE_VMNET_HOST:
                ini_section_set_string(cat, temp, "vmnet-host");
                break;
            case NET_TYPE_VMNET_BRIDGE:
                ini_section_set_string(cat, temp, "vmnet-bridged");
                break;
